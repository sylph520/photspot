...
var str = '<script type="text/javascript" src="js/jscharts.js"><\/script>'; 
document.write(str);
    ...
    Bubble.prototype.generatePaths = function (time) {
        ...
        for (var i = 0, l = dots.length; i < l; i++) {
            var dot = dots[i],
				...
				var nodeText = foreCanvas.text(x, y, that.keys[i]).attr({
        'opacity': 1,
        //'font-family': "微软雅黑",
        'font': '12px Verdana'
      });
        ...
        dotBubbleSet.forEach(function (d, i) {
            (function (d, i) {
                ...
                d.click(function() 
				{
					...
					$("#popupMsg").css({top: y, left: x});
					$("#popupMsg").fadeTo("fast", 0.66);
			
			
					var myData = new Array([1997, 7.80], [1998, 4.80], [1999, 6.50], [2000, 6.10], [2001, 4.40], [2002, 5.80], [2003, 4.00], [2004, 8.50], [2005, 8.90], [2006, 9.20], [2007, 22], [2008, 36], [2009, 37], [2010, 45], [2011, 50], [2012, 55], [2013, 61], [2014, 61], [2015, 62], [2016, 66], [2017, 73]);
					var myChart = new JSChart('popupMsg', 'line');
					myChart.setDataArray(myData);
					myChart.setTitle('专利趋势预测');
